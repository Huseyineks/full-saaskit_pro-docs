...
description: "Deep dive into SaaSKit's permission layers, dynamic filter guards, and automated background workers."
---
...
* **PermissionScope.System:** These permissions are strictly confined to system users operating under the global administration realm (e.g., `plan.create`, `tenants.read`, `features.create`). They cannot be visualized, assigned, or manipulated by standard tenant administrators.
* **PermissionScope.Tenant:** Capabilities available within the active tenant context and, where needed, the system workspace (e.g., `user.read`, `role.create`, `invoice.read`).
...
* **Hierarchical Settings Check:** Before writing to the database, it queries the active tenant's `AdvancedSettings`. If audit logging is disabled for that tenant, it immediately checks the global system settings. If the system switch is enabled but the tenant has it turned off, the log is written globally but marked as decoupled from that specific tenant (`TenantId = null`).
...
#### 🧹 Tenant Log Retention Service

* **Execution Boundary:** Runs on an automated 8-hour tick loop.
* **Core Mechanics:** Enforces custom retention schedules. It evaluates tenant configuration profiles, looks up their defined `LogRetentionDays` limit, and purges older records from the `AuditLog` and `ActivityLog` schemas to prevent unbounded database storage inflation.
...
## 🏢 5. Tenant Context Provisioning & Subscription Onboarding
...
### 🗄️ Tenant Context Filtering (EF Core Query Filters)
